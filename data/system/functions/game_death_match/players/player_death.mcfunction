...
execute if score phase control matches 2 at @e[type=minecraft:armor_stand,name=center] run spreadplayers ~ ~ 0 50 false @a[scores={death=1}]
scoreboard players set @a[scores={death=1}] death 2

execute if score phase control matches 2 as @a[scores={death=2,respawn=0}] at @s unless entity @a[scores={join=4},distance=..10,limit=1] at @e[type=minecraft:armor_stand,name=center] run spreadplayers ~ ~ 0 50 false @s
